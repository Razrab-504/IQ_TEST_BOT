# 🧠 IQ Test Bot

Telegram-бот для создания и прохождения тестов с системой статистики и аналитики.

## 📋 Описание

Бот позволяет администраторам создавать неограниченное количество тестов, а пользователям — проходить их и отслеживать результаты. Реализована детальная статистика и история попыток.

## ✨ Возможности

### Для пользователей:
- ✅ Прохождение тестов с мгновенной проверкой ответов
- 📊 Просмотр истории всех попыток
- 📈 Статистика по каждому тесту

### Для администраторов:
- ➕ Создание тестов с описанием
- ❓ Добавление вопросов (формат: A/B/C)
- 🗑️ Удаление тестов
- 📊 Общая статистика: попытки, уникальные пользователи, средний балл

## 🛠 Технологии

- **Python 3.10+**
- **aiogram 3.x** — асинхронный фреймворк для Telegram Bot API
- **SQLAlchemy 2.0** — ORM с async поддержкой
- **PostgreSQL** — реляционная база данных
- **asyncpg** — драйвер для PostgreSQL
- **FSM** — управление состояниями диалога

## 📦 Установка

1. **Клонируйте репозиторий:**
```bash
git clone https://github.com/yourusername/IQ_Test_bot.git
cd IQ_Test_bot
```

2. **Создайте виртуальное окружение:**
```bash
python -m venv venv
source venv/bin/activate  # для Linux/Mac
venv\Scripts\activate     # для Windows
```

3. **Установите зависимости:**
```bash
pip install -r requirements.txt
```

4. **Создайте файл `.env`:**
```env
BOT_TOKEN=your_bot_token_here
DATABASE_URL=postgresql+asyncpg://user:password@localhost/dbname
ADMIN_IDS=123456789,987654321
```

5. **Создайте базу данных:**
```bash
# В PostgreSQL
createdb iq_test_db
```

6. **Инициализируйте таблицы:**
```bash
python -m src.db.create_tables
```

7. **Запустите бота:**
```bash
python main.py
```

## 📁 Структура проекта
```
IQ_Test_bot/
├── src/
│   ├── bot/
│   │   ├── handlers/
│   │   │   ├── admin_handlers.py    # Хендлеры для админов
│   │   │   └── user_handlers.py     # Хендлеры для пользователей
│   │   ├── keyboards/
│   │   │   ├── admin_keyboard.py    # Клавиатуры админа
│   │   │   └── user_keyboard.py     # Клавиатуры пользователя
│   │   └── filters/
│   │       ├── admin_filter.py      # Фильтр для админов
│   │       └── user_filter.py       # Фильтр для пользователей
│   └── db/
│       ├── config.py                # Конфигурация БД
│       ├── database.py              # Настройка подключения
│       ├── create_tables.py         # Создание таблиц
│       ├── models.py                # Модели SQLAlchemy
│       └── crud.py                  # CRUD операции
├── main.py                          # Точка входа
├── requirements.txt
├── .env.example
└── README.md
```

## 🗄️ Структура базы данных

### Таблицы:
- **users** — пользователи бота
- **tests** — созданные тесты
- **questions** — вопросы тестов
- **choices** — варианты ответов (A, B, C)
- **attempts** — попытки прохождения тестов
- **answers** — ответы пользователей на вопросы

### Связи:
- `tests` ← `questions` (один ко многим)
- `questions` ← `choices` (один ко многим)
- `users` ← `attempts` (один ко многим)
- `tests` ← `attempts` (один ко многим)
- `attempts` ← `answers` (один ко многим)

## 🎯 Примеры использования

### Создание теста (админ):
```
1. Нажать "➕ Создать тест"
2. Ввести название: "IQ Test"
3. Ввести описание: "Проверь свой уровень IQ"
4. Добавить вопросы в формате:

2+2=?
Варианты: A) 3 B) 4 C) 5
Правильный ответ: B)

5. Отправить /done (минимум 5 вопросов)
```

### Прохождение теста (пользователь):
```
1. Нажать "📝 Показать все тесты"
2. Выбрать тест
3. Отвечать на вопросы, нажимая A, B или C
4. Получить результат и статистику
```

## 🔧 Конфигурация

Создайте файл `.env` в корне проекта:
```env
# Токен бота от @BotFather
BOT_TOKEN=123456789:ABCdefGHIjklMNOpqrsTUVwxyz

# URL подключения к PostgreSQL
DATABASE_URL=postgresql+asyncpg://user:password@localhost:5432/iq_test_db

# Telegram ID админов (через запятую)
ADMIN_IDS=123456789,987654321
```

### Как получить Telegram ID:
1. Напишите боту [@userinfobot](https://t.me/userinfobot)
2. Скопируйте ваш ID

## 📊 Статистика

**Админы видят по каждому тесту:**
- Общее количество прохождений
- Количество уникальных пользователей
- Средний балл из максимального

**Пользователи видят:**
- Список пройденных тестов
- Все свои попытки с баллами
- Дату и время прохождения

## 🚀 Деплой

### На сервере (Linux):

1. **Установите PostgreSQL:**
```bash
sudo apt update
sudo apt install postgresql postgresql-contrib
```

2. **Создайте базу данных:**
```bash
sudo -u postgres psql
CREATE DATABASE iq_test_db;
CREATE USER botuser WITH PASSWORD 'yourpassword';
GRANT ALL PRIVILEGES ON DATABASE iq_test_db TO botuser;
\q
```

3. **Клонируйте и настройте проект**
4. **Запустите через systemd или screen**

### Docker (опционально):
```dockerfile
FROM python:3.10-slim

WORKDIR /app

COPY requirements.txt .
RUN pip install --no-cache-dir -r requirements.txt

COPY . .

CMD ["python", "main.py"]
```
```yaml
# docker-compose.yml
version: '3.8'

services:
  bot:
    build: .
    env_file: .env
    depends_on:
      - db
    restart: unless-stopped

  db:
    image: postgres:15
    environment:
      POSTGRES_DB: iq_test_db
      POSTGRES_USER: botuser
      POSTGRES_PASSWORD: yourpassword
    volumes:
      - postgres_data:/var/lib/postgresql/data

volumes:
  postgres_data:
```

Запуск:
```bash
docker-compose up -d
```

## 📝 TODO

- [ ] Добавить таймер для тестов
- [ ] Поддержка изображений в вопросах
- [ ] Экспорт статистики в Excel/PDF
- [ ] Рейтинг пользователей
- [ ] Многоязычность (i18n)
- [ ] Обработка ошибок с логированием
- [ ] Unit-тесты для CRUD операций

## 🐛 Известные проблемы

- Нет обработки отключения от БД
- При незавершённом тесте создаются записи со статусом `in_progress`

## 🤝 Контрибьюция

Pull requests приветствуются! Для больших изменений сначала откройте issue.

1. Fork проекта
2. Создайте feature branch (`git checkout -b feature/AmazingFeature`)
3. Commit изменения (`git commit -m 'Add some AmazingFeature'`)
4. Push в branch (`git push origin feature/AmazingFeature`)
5. Откройте Pull Request

## 📄 Лицензия

MIT License

## 👤 Автор

Ваше имя — [Telegram](https://t.me/yourusername) | [GitHub](https://github.com/yourusername)

---

⭐️ Если проект понравился, поставьте звезду на GitHub!